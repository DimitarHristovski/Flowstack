...
export default function DashboardPage() {
  ...
  const sidebarLinks = [
    { to: '/dashboard', icon: <Home size={20} />, label: t('nav.dashboard'), exact: true },
    { to: '/dashboard/agents', icon: <Bot size={20} />, label: t('dashboard.agents.title') },
      ...
      <div className="bg-white dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700 py-3 px-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <div className="relative">
            ...
            >
              <Bell size={20} />
              <span className="absolute top-1 right-1 w-4 h-4 bg-error-500 text-white text-xs flex items-center justify-center rounded-full">3</span>
            </button>
            ...
            {isNotificationsOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-surface-800 rounded-md shadow-lg border border-surface-200 dark:border-surface-700 z-10">
                <div className="p-3 border-b border-surface-200 dark:border-surface-700 flex justify-between items-center">
                  ...
                  <span className="bg-error-500 text-white text-xs px-2 py-1 rounded-full">3 new</span>
                </div>
                <div className="max-h-96 overflow-y-auto">
                  <div className="p-3 border-b border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-700 cursor-pointer">
                    <div className="flex items-start">
                      ...
                      <div>
                        <p className="text-sm text-surface-900 dark:text-white font-medium">New agent available</p>
                        <p className="text-xs text-surface-500 dark:text-surface-400 mt-1">Content Writer Pro now supports multiple languages</p>
                        <p className="text-xs text-surface-400 dark:text-surface-500 mt-1">10 minutes ago</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-surface-800 rounded-md shadow-lg border border-surface-200 dark:border-surface-700 z-10">
                <div className="p-3 border-b border-surface-200 dark:border-surface-700">
                  <div className="font-medium text-surface-900 dark:text-white">{profile?.full_name || 'User'}</div>
          ...
          <div className="fixed inset-0 bg-surface-900/50 backdrop-blur-sm z-50 flex lg:hidden">
            ...
            >
              <div className="p-4 border-b border-surface-200 dark:border-surface-700 flex justify-between items-center">
                <h3 className="font-medium text-surface-900 dark:text-white">Menu</h3>
                <button
